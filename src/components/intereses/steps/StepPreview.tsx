...
import InteresesTemplate from '../InteresesTemplate';

interface StepPreviewProps {
    formData: InteresesFormData;
    resultado: ResultadoLiquidacion;
    onBack: () => void;
}

async function generatePDF(resultado: ResultadoLiquidacion): Promise<void> {
    const [{ pdf }, { default: InteresesPDF }] = await Promise.all([
        import('@react-pdf/renderer'),
        import('../InteresesPDF'),
    ]);

    const blob = await pdf(<InteresesPDF resultado={resultado} />).toBlob();

    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'liquidacion-intereses.pdf';
...
export default function StepPreview({ resultado, onBack }: StepPreviewProps) {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                    <h2 className="text-xl font-black text-secondary">Liquidación lista</h2>
                    <p className="text-sm text-slate-500">
                        Intereses {resultado.tipo === 'corriente' ? 'corrientes' : 'moratorios'} · Superfinanciera
                        Colombia
                    </p>
                </div>
                <Button
