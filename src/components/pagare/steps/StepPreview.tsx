...
async function generatePDF(formData: PagareFormData): Promise<void> {
    const [{ pdf }, { default: PagarePDF }] = await Promise.all([
        import('@react-pdf/renderer'),
        import('../PagarePDF'),
    ]);

    const blob = await pdf(<PagarePDF formData={formData} />).toBlob();
...
export default function StepPreview({ formData, onBack }: StepPreviewProps) {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            >
                <PagareTemplate formData={formData} />
            </div>
