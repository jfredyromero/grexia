...
async function generatePDF(formData: PoderFormData): Promise<void> {
    const { pdf } = await import('@react-pdf/renderer');
    const { default: PoderEspecialPDF } = await import('../pdf/PoderEspecial');

    const blob = await pdf(<PoderEspecialPDF formData={formData} />).toBlob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'poder-especial.pdf';
...
export default function StepPreview({ formData, onBack }: StepPreviewProps) {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <h2 className="text-xl font-black text-secondary">Tu poder está listo</h2>
                    </div>
                    <p className="text-sm text-slate-500">
                        Poder Especial · Art. 74 y 77 del Código General del Proceso
                    </p>
                </div>
                <Button
            ...
            >
                <PoderEspecial formData={formData} />
            </div>
